$(function () {
  ...
  // custom uiDropdown element, example can be seen in user-list.html on the 'Filter users' button
    ...
    $(".userDelete").click(function(e){
      ...
      $('#userDeleteModal #deleteBtn').attr('href', url);
      return false;
    });
    $(".treatmentDelete").click(function(e){
      e.preventDefault;
      var url = $(this).attr('href');
      $("#treatmentDeleteModal").modal();
      $('#treatmentDeleteModal #deleteBtn').attr('href', url);
      return false;
    });
    $(".testimonialDelete").click(function(e){
      e.preventDefault;
      var url = $(this).attr('href');
      $("#testimonialDeleteModal").modal();
      $('#testimonialDeleteModal #deleteBtn').attr('href', url);
      return false;
    });
    
    // TABLE DRAG AND DROP
    $(".table.table-order").tableDnD({
	    onDragClass: 'dragRow',
	    onDragStart: function(table, row){
		    $(row).addClass('dragRow');
	    },
	    onDrop: function(table, row) {
            $(row).removeClass('dragRow');
            ...
            var jsonString = $(order).serialize();
            var href = $('.table.table-order').attr('rel');
            console.log(href);
            $.ajax({
		            type: "POST",
		            url : href, 
		            ...
		            beforeSend: function(XMLHttpRequest) {
		                $('.message_order').show();
		                $(".table.table-order tr").addClass("nodrag nodrop");
		                $(".table.table-order tr").css("cursor", "not-allowed");

		            },
		            success: function(data) {
		            	$('.message_order').hide();
		            	$(".table.table-order tr").removeClass("nodrag nodrop");
		            	$(".table.table-order tr").css("cursor", "move");
		            },
		            error: function(XMLHttpRequest, textStatus, errorThrown) {
		            			alert("Aconteceu um problema ao ordenar a tabela. Por favor tente novamente.");
					            $(".table.table-order tr").removeClass("nodrag nodrop");
		            }
           }); 
        }
